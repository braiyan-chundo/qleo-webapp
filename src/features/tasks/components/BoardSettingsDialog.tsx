...
/**
 * Configuración del tablero fuera del flujo principal (board-first): un diálogo con las
 * columnas de estado (QL-06) para no dominar la vista del board. Añade (QL-63) un bloque de
 * configuración (Backlog/inicio/fin/plantilla) visible para quien puede gestionar el proyecto
 * (`canManageProject`: ADMIN, creador o gestor otorgado) — la MISMA regla que gatea el botón
 * que abre este diálogo; el backend valida igual.
 */
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* `DialogContent` es una rejilla (`display:grid`): sus hijos tienen `min-width:auto`, así
          ...
          diálogo; el panel ya no aplasta sus filas (ver `ColumnsPanel`). */}
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader className="min-w-0">
          <DialogTitle>Configurar tablero</DialogTitle>
          <DialogDescription>
            Define las columnas de estado del Kanban.
          </DialogDescription>
        </DialogHeader>
